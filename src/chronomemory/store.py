...
class ChronoStore:
    ...
    def compact(self) -> int:
        ...
        # Reset _last_hash to "" so the sentinel becomes a new hash-chain
        # genesis, keeping chain_valid() correct after compact.
        self._last_hash = ""
    ...
    def _replay_wal(self) -> None:
        ...
        All per-line errors (unparseable JSON, wrong seq type, missing fields)
        are silently skipped so a single bad line never prevents replay of
        the rest of the WAL.
        """
        if not self._wal_path.exists():
            return
        ...
        for line in lines:
            ...
            try:
                ...
                # Guard against non-integer seq (e.g. WAL tampered with string)
                try:
                    seq = int(event.seq)
                ...
                continue  # Skip any malformed / unprocessable line
