...
This guide walks through the core workflow: writing beliefs, querying them, verifying integrity, and migrating from legacy JSON.
...
store.upsert(ChronoRecord(
    id="HYP-001",
    kind="hypothesis",
    label="CTT R=4→R=6 generalization gap < 5% of Bayes-optimal",
    is_hypothesis=True,
    confidence=0.75,
...
Or via CLI: `specsmith esdb migrate` (requires specsmith ≥ 0.11.3).
