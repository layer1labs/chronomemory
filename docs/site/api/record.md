...
```python
rec = ChronoRecord(id="X", label="test")
d = rec.to_dict()
print(d["id"])  # "X"
```
...
```python
rec = ChronoRecord.from_dict({"id": "X", "label": "test", "unknown_field": "ignored"})
```
...
rec3 = ChronoRecord(id="Z", confidence=0.9, status="tombstone")
print(rec3.passes_rag_filter())  # False
```
...
rec = ChronoRecord(
    ...
    confidence=1.0,
    evidence=["seed=42", "model=CTTStateNet", "scenario=DoorKey-5x5", "epoch=50"],
    epistemic_boundary=["model:CTTStateNet-v1", "dataset:DoorKey-5x5"],
    data={"invalid_rate": 0.0, "nll_bits": 0.312, "task_success": 1.0},
)
...
rec = ChronoRecord(
    id="HYP-scaling-001",
    kind="hypothesis",
    label="Scaling CTT to K=1000 constraints keeps O(K) memory",
    is_hypothesis=True,
...
The `status` field in `ChronoRecord` is the ESDB lifecycle status — it is **separate** from governance lifecycle statuses (`defined`, `implemented`, etc.) used by specsmith.
