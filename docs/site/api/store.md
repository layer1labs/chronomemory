...
Tombstone a record. Status is set to `"tombstone"` in memory and a delete event is appended to the WAL. The record is never physically removed.
...
| `status` | `str` | `"active"` | Filter by status. Pass `""` to include all statuses (including tombstones). |
| `rag_filter` | `bool` | `False` | Apply H18 confidence filter (`confidence >= 0.6`). |
| `min_confidence` | `float` | `0.0` | Minimum confidence threshold. Takes max with `rag_filter` threshold. |
...
```python
# All events
all_events = store.replay(from_seq=0)

# Only new events since seq 100
new_events = store.replay(from_seq=101)
```
...
- Governance statuses (`implemented`, `defined`, etc.) map to ESDB `active`; `deprecated` maps to `deprecated`.
...
`ChronoStore` is safe to use from **a single thread only**. For concurrent reads, create a separate instance per reader (each `open()` replays from disk independently). Do not share a single instance across threads.
