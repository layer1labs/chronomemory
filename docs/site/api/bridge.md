...
2. **JSON fallback** — no WAL yet → reads `.specsmith/requirements.json` and `testcases.json` directly

This allows tooling (specsmith CLI, dashboards, CI scripts) to work with both migrated and non-migrated projects without branching.
...
Write or update a record. Returns `True` if ESDB is active and the write succeeded. Returns `False` if only the JSON fallback is available.

```python
from chronomemory.bridge import EsdbRecord

rec = EsdbRecord(id="FACT-001", kind="fact", label="A new fact", confidence=0.9)
success = bridge.upsert_record(rec)
```
